...
def get_user_by_id(user_id):
    return jsonify([user.json() for user in User.get_by_id(user_id)])


@user_api_blueprint.route('/<string:user_type>')
@requires_login
def get_user_by_type(user_type):
    return jsonify([user.json() for user in User.get_by_type(user_type)])


@user_api_blueprint.route('/<string:user_type>')
@requires_login
def get_active_users_by_type(user_type):
    return jsonify([user.json() for user in User.get_active_by_type(user_type)])
